...
var amount_of_parts = 4
var new_array = []
for (let i = 0; i < amount_of_parts; i++) {
  new_array.push(0);
}
...
//нам нужна функция, которая выдает рандом неповторяющийся и до определенного предела
//т,е функция у нас создаем определенный массив заданной длины
// каждый вызов рандома отнимает элемент массива,
//если число уже такое выпадало, тогда функция перезапускается
// function getRandomInt(max) {
//     return Math.floor(Math.random() * Math.floor(max));
//   }

// function uniqRandom(amount_of_parts){
//     let rand = getRandomInt(amount_of_parts);
//     if (new_array[rand] == null){
//         uniqRandom(amount_of_parts);
//     } else{
//         new_array[rand] = null;
//         return rand;
//     }
...
setTimeout("gameCycle()", DELAY) //проигрывает цикл игры
  ...
  function loadImages(){
    ...
    for (let i = 0; i < amount_of_parts; i++) {
        part = new Image();
        part.src = `dot_${i}.png`; // загрузка секции тела
        parts.push(part)
        console.log(part)
    }
    ...
    if(inGame){
        ...
        for(var z = 0; z < dots; z++){
            if(z ==0){
                ...
                ctx.drawImage(parts[cikl], x[z], y[z])
                cikl++;
                if (cikl == amount_of_parts ){
                    cikl = 0;
                }
            }
        }
